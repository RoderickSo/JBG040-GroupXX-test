...
#additions 
from dc1.additions import compute_sensitivity
from dc1.additions import compute_accuracy
from dc1.additions import compute_false_negative_rate
...
def main(args: argparse.Namespace, activeloop: bool = True) -> None:
    ...
    # retrieves sensitivity so TP/(TP+FN)
    sens = compute_sensitivity(model, test_sampler, device)
    for i, s in enumerate(sens):
        print(f"Class {i} sensitivity: {s:.3f}")

    #retrieves accuracy TP+TN/T
    acc = compute_accuracy(model, test_sampler, device)
    print(f"Test Accuracy: {acc:.3f}")

    #retrieves false negative rate FN/(TP+FN)
    fnr = compute_false_negative_rate(model, test_sampler, device)
    for i, value in enumerate(fnr):
        print(f"Class {i} FNR: {value:.3f}")
